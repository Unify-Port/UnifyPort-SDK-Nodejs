...
[English](../../api-reference/messages.md) | [简体中文](messages.md)
...
| `body.reply_to.reply_token`                | 父字段存在时必填 | `string`                                                                                    | -                                                                     | 来自入站事件 `data.message.reply_token` 或发送成功后 `SendMessageResult.reply_token` 的不透明句柄；调用时应原样回传。 |
...
| `result.data.data.reply_token`  | 否   | `string`            | -    | WhatsApp 发送成功后可能同步返回不透明引用句柄；无法生成稳定句柄时省略。该值按敏感数据处理，不要写入日志。 |
