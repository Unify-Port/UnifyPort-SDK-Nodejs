...
`@unifyport/sdk-node` `0.1.3` 已按 MIT 许可证发布到 npm。`@unifyport/mcp-server` 仍保持 `private: true`；发布 SDK 不会扩大 MCP Server 的分发边界。
...
发布前基线在 Node.js 24.14.0、pnpm 11.7.0 环境执行：
...
另在不包含 `.git`、`node_modules`、`dist` 与 `coverage` 的临时副本中执行 `pnpm install --frozen-lockfile && pnpm clean && pnpm check`，fresh checkout 场景完整通过。
...
- `@unifyport/sdk-node@0.1.3` 已发布到 npm，并在 Git 中标记为 `v0.1.3`；
